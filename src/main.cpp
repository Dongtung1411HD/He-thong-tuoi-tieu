...
int re1 = 0; int re2 = 0; int re3 = 0; int re4 = 0; int re5 = 0; //RELAY

int cb = A2;      //Chân cảm biến ở chân Analog: A0
int doc_cb, TBcb;
int doam= 0;
int phantramao;
int phantramthuc;
...
if (gtlen != macdinh)  // NÚT LÊN
{
  if (gtlen == 0) // Khi nhấn nút lên
  {
    ...
    else if (demtong == 2)    //+ Con trỏ Set Date/Time và lên Menu Điều khiển
    {
      ...
      if(congtru_tong == 4) // + CON TRỎ ở SET DATE và TIME
      {
        contro ++;
        if (contro > 15)
          contro = 0;
    ...
    else if (demtong == 3)   
    {
      ...
      else if(congtru_tong == 4) // - ở GIỜ : PHÚT : GIÂY
      {
        if ((contro == 9 or contro == 10)){ //GIỜ
          doam -=5;
          if (doam < 0)
            doam = 99;}
      }
...
{  
  ...
  {  
    ...
    else if (demtong == 2) 
    {
      ...
      else if(congtru_tong == 4) //Menu ĐIỀU KHIỂN
      {
        do_am();
      }
    }                                           
    ...
    else if (demtong == 4) //Thoát CON TRỎ
    {
      ...
      else if(congtru_tong == 4) //Thoát CON TRỎ từ SET TIME ra
      {
        demtong = 2;
        chonmenu_tong(); 
      }
...
re1 = 0;
//  re2 = 0; re3 = 1; re4 = 0; re5 = 1; //RELAY

ton1 = 23; pon1 = 1; tof1 = 23; pof1 = 02; // 1 
// ton2 = 22; pon2 = 6; tof2 = 22; pof2 = 7; // 0
// ton3 = 22; pon3 = 7; tof3 = 22; pof3 = 8; // 1
// ton4 = 22; pon4 = 8; tof4 = 22; pof4 = 9; // 0
// ton5 = 22; pon5 = 9; tof5 = 22; pof5 = 10; // 1
...
else if ((demtong == 2 or demtong == 3) && congtru_tong != 3) //SETUP DATE / TIME
{
  ...
  else if (congtru_tong == 4) 
  {
    ...
    lcd.setCursor(10,1); lcd.print(doam);
    }
    else {
      lcd.setCursor(9,1); lcd.print(doam);
    }
    lcd.setCursor(contro ,1); 
    lcd.cursor();
    delay(50); 
    ...
    lcd.setCursor(10,0); lcd.print(phantramthuc);
    }
    else {
      lcd.setCursor(9,0); lcd.print(phantramthuc);
    }
  }
}
...
{ 
  ...
  // Serial.print("Con trỏ: "); Serial.println(contro);
    ...
    phantramao = map(TBcb, 0, 1023, 0, 100);    
    phantramthuc = 100 - phantramao;  

    doc_cb = 0;
    if ( phantramthuc <= doam )         //Nếu phần trăm thực lớn hơn hoặc = 30%

    {
      digitalWrite (relay1, LOW);    //Thì relay OFF, không bơm
    }

    else                              //Ngược lại, phần trăm thực nhỏ hơn 30%
    {
      digitalWrite (relay1, HIGH);   //Thì relay ON, Bơm chạy
    }
